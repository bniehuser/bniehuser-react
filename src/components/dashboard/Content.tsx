...
const AddListing: FC<ListingProps> = (p) => (
  <div className={'panel-option'}>
    <div className={'panel-option-title'}>{p.name}</div>
    <div className={'panel-option-input'}>{p.opts}</div>
...
export const Content: FC<Props> = ({ config, parent }) => {
  ...
  const findParent = (
    n: DashNode | DashLeaf,
    d: DashNode = dash,
  ): DashNode | DashLeaf | undefined =>
    d.nodes?.indexOf(n) > -1 ? d : d.nodes?.reduce((a, s) => a || findParent(n, s as DashNode));
  const split = (direction: 'row' | 'column') => {
    if (parent.orientation === direction) {
      const size = parseFloat((config?.size as string) || '0') / 2 + '%';
      setDashNode(config, [
        { ...config, size } as DashLeaf,
        { size, name: 'Select...', component: '', arguments: {} } as DashLeaf,
      ]);
    } else {
      setDashNode(config, [
        {
          ...
          nodes: [
            { ...config, size: '50%' },
            { size: '50%', component: '', name: 'Select...', arguments: {} },
          ],
        },
      ]);
    }
  ...
  const deleteContent = () => {
    ...
    if (up) {
      ...
      const nodes = (up as DashNode).nodes
        .filter((n) => n !== lc)
        .map((n) => ({
          ...n,
  ...
  if (parent.orientation === 'row') {
    styles.width = config.size;
  }
  if (parent.orientation === 'column') {
    styles.height = config.size;
  }
  const type = config?.component;
  return (
    <div className={'dash-leaf'} style={{ ...styles, position: 'relative', minHeight: 0 }}>
      <div style={{ display: 'flex', minHeight: 0, flexDirection: 'column' }}>
        <div className={'dash-panel-title'} style={{ minHeight: 0 }}>
          {config?.name || 'select contents'}
          <div className={'dash-panel-button-tray'}>
            <button className={'dash-panel-button hidden'} onClick={splitHorizontal}>
              —
            </button>
            <button className={'dash-panel-button hidden'} onClick={splitVertical}>
              |
            </button>
            <button className={'dash-panel-button hidden'} onClick={deleteContent}>
              ✖
            </button>
            <button className={'dash-panel-button'}>⚙</button>
          </div>
        </div>
        <div className={'dash-panel-content scrollable'} style={{ minHeight: 0 }}>
          {(() => {
            switch (type) {
              case 'recipe-search':
                return (
                ...
                return (
                  <div className={'panel-selector'}>
                    <AddListing
                      ...
                      onClick={() =>
                        setDashNode(config, [
                          {
                            ...config,
                            name: `Recipe Search: ${newRecipeSearch}`,
                            component: 'recipe-search',
                            arguments: { search: newRecipeSearch },
                          },
                        ])
                      }
                    />
                    <AddListing
                      ...
                      onClick={() =>
                        setDashNode(config, [
                          {
                            ...config,
                            name: `Stock: ${newTicker}`,
                            component: 'stock',
                            arguments: { ticker: newTicker },
                          },
                        ])
                      }
                    />
                    <AddListing
                      ...
                      onClick={() =>
                        setDashNode(config, [
                          { ...config, name: 'Public Chat', component: 'publicChat' },
                        ])
                      }
                    />
                    <AddListing
                      ...
                      onClick={() =>
                        setDashNode(config, [
                          { ...config, name: 'Private Chat', component: 'privateChat' },
                        ])
                      }
                    />
                    <AddListing
                      ...
                      opts={
                        <>
                          ...
                          <input
                            ...
                            size={2}
                            style={{ width: '50px' }}
                            value={newRandomParagraphs}
                            onChange={(e) => setNewRandomParagraphs(e.target.value)}
                          />
                          <span style={{ fontSize: '75%' }}>(blank for random)</span>
                        </>
                      }
                      onClick={() =>
                        setDashNode(config, [
                          {
                            ...config,
                            name: `Random${newRandomParagraphs ? `: ${newRandomParagraphs}` : ''}`,
                            component: 'random',
                            arguments: {
                              paragraphs: newRandomParagraphs
                                ? parseInt(newRandomParagraphs, 10)
                                : undefined,
                            },
                          },
                        ])
      ...
      <div
        className={'panel-bottom-shim'}
        onMouseDown={(e) => startResize(e, config, 'horizontal')}
      />
    </div>
  );
};
