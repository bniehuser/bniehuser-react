...
export const StockView: FC<Props> = ({ data, ticker: tick }) => {
  ...
  const [series, setSeries] = useState<{ series: SeriesData[]; linear: SeriesData[] }>({
    series: [],
    linear: [],
  });
  ...
  const {
    ...
    error,
    inProgress,
  } = useApiMethod(StocksService.getTickerStocksTickerGet);
      ...
      <>
        <div style={{ display: 'flex', flexDirection: 'column', margin: '.25em' }}>
          <div style={{ display: 'flex', flexDirection: 'row' }}>
            <div style={{ marginRight: '.5em', marginBottom: '.25em' }}>
              <img src={stock.logo_url} alt={stock.name_short} />
            </div>
            <div>
              <h2 style={{ marginBottom: '.25em' }}>{stock.symbol}</h2>
              <strong style={{ display: 'block', marginBottom: '1em' }}>{stock.name}</strong>
              <div>
                <strong style={{ fontSize: '200%', paddingRight: '.5em' }}>
                  <Number currency={true}>{stock.price}</Number>
                </strong>
                <span style={{ fontSize: '150%' }}>
                  <Number currency={true} colorize={true}>
                    {stock.change}
                  </Number>
                </span>
              </div>
        ...
        {series.series?.length && (
          ...
          >
            <StockChart seriesData={series.series} seriesDataLinear={series.linear} />
          </div>
        )}
      </>
    );
