        ...
        recipeResults.map((r) => (
          <div className={'panel-option'}>
            <div className={'panel-option-input'}>
              <h4>{r.title}</h4>
              <span>
                used: <em>{r.used_ingredients.map((i) => i.name).join(', ')}</em>
              </span>{' '}
              <span>
                unused: <em>{r.unused_ingredients.map((i) => i.name).join(', ')}</em>
              </span>{' '}
              <span>
                needed: <em>{r.missed_ingredients.map((i) => i.name).join(', ')}</em>
              </span>
            </div>
